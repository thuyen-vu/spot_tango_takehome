...
const Products = ({ group }) => {
  ...
  const [selectedAvailability, setSelectedAvailability] = useState(""); // ✅ NEW
  ...
  useEffect(() => {
    group ? setSelectedGroups([group]) : setSelectedGroups([]);
  }, [group]);

  // ✅ Combined filtering for Category + Availability
  const filteredProducts = productsData.filter((product) => {
    const matchesGroup =
      selectedGroups.length === 0 || selectedGroups.includes(product.group);
    ...
    <div className="max-w-7xl mx-auto px-5">
      ...
      <div className="items-start flex">
        ...
        <div>
          ...
          />
          <main className="flex-1 grid gap-x-4 gap-y-20 sm:grid-cols-2 md:grid-cols-3 mb-10">
            {errorMessage ? (
              <p>{errorMessage}</p>
            ) : (
                    ...
                    <div className="flex justify-center inline-block">
                      ...
                      {product.status === "Available" ? (
                        <button
                          ...
                          className="cursor-pointer text-white bg-black hover:bg-gray-900 rounded-full py-2 w-full mt-5 "
                        >
                          Add to Cart
                        </button>
                      ) : (
                        <button className="text-white bg-sky-950 rounded-full py-2 w-full mt-5">
                          SOLD OUT
                        </button>
                      )}
                    </div>
